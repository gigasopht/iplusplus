// Win+ -- BlockOut2.cpp -- Mouse Button & Capture Demonstation Program
    ...
    int show_command)
{
    ...
    handle window = create_window(atom_name, L"Mouse Button Demonstration - with Capture");
    ...
    parameter parameter2)
{
    ...
    {
        window_data* data = (window_data*)get_window_pointer(window_handle, 0);
        if (data->blocking & (int)parameter1 == '\x1B')       // i.e., Escape
        {
            draw_box_outline(window_handle, data->start, data->end);
            release_capture();
            set_cursor(load_cursor((handle)null, (const character*)cursor_identity::arrow));
