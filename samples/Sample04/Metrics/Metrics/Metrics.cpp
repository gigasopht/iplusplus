    ...
    int show_command)
{
    ...
    handle window = create_window(atom_name, L"System Metrics");
    ...
    parameter parameter2)
{
    ...
    case message::vertical_scroll:
    {
        ...
        if (data->increment_of_vertical_scroll != 0)
        {
            ...
            scroll_window(window_handle, 0, -data->height_of_character * data->increment_of_vertical_scroll);
            set_scroll_position(window_handle, scrollbar_identity::vertical, data->position_of_vertical_scroll, true);
    ...
    case message::horizontal_scroll:
    {
        ...
        if (data->increment_of_horizontal_scroll != 0)
        {
            ...
            scroll_window(window_handle, -data->width_of_character * data->increment_of_horizontal_scroll, 0);
            set_scroll_position(window_handle, scrollbar_identity::horizontal, data->position_of_horizontal_scroll, true);
        }
    }
    break;
    ...
    case message::paint:
    {
        ...
        int paint_begin = maximum(0, (int)(data->position_of_vertical_scroll + paint_structure.bounds[0](1) / data->height_of_character - 1));
        int paint_end = minimum((int)lines, (int)(data->position_of_vertical_scroll + (int)paint_structure.bounds[1](1) / data->height_of_character));
        ...
        for (int i = paint_begin; i < paint_end; i++)
        {
            ...
            text_out(device_context,
                x,
                y,
                (const character*)metrics[i].title,
                string_length((const character*)metrics[i].title));

            text_out(device_context,
                x + column1 * data->width_of_capitals,
                y,
                (const character*)metrics[i].description,
                string_length((const character*)metrics[i].description));
    ...
    case message::key_down:
        ...
        case virtual_key::home:
            send_message(window_handle, message::vertical_scroll, (parameter)scrollbar_notify::top);
            break;

        case virtual_key::end:
            send_message(window_handle, message::vertical_scroll, (parameter)scrollbar_notify::bottom);
            break;

        case virtual_key::prior:
            send_message(window_handle, message::vertical_scroll, (parameter)scrollbar_notify::page_up);
            break;

        case virtual_key::next:
            send_message(window_handle, message::vertical_scroll, (parameter)scrollbar_notify::page_down);
            break;

        case virtual_key::up:
            send_message(window_handle, message::vertical_scroll, (parameter)scrollbar_notify::line_up);
            break;

        case virtual_key::down:
            send_message(window_handle, message::vertical_scroll, (parameter)scrollbar_notify::line_down);
            break;

        case virtual_key::left:
            send_message(window_handle, message::horizontal_scroll, (parameter)scrollbar_notify::page_up);
            break;

        case virtual_key::right:
            send_message(window_handle, message::horizontal_scroll, (parameter)scrollbar_notify::page_down);
            break;
        }
        break;
